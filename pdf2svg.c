...
int main(int argn, char *args[])
{
    ...
    if(pageLabel == NULL) {
        ...
        else if (strcmp(pageLabel, "split") == 0) {
            gchar *svgFilePrefix = g_strdup(svgFilename);
            size_t svgFilenameBufLen = strlen(svgFilename);
            svgFilePrefix[svgFilenameBufLen - 4] = '\0';
            int pageCount = poppler_document_get_n_pages(pdffile);
